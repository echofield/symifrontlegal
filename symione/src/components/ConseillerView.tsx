    ...
    <div className="min-h-screen pt-16">
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Right: Lawyer search + results (no map) */}
            ...
            {lawyers.length > 0 && (
              <div className="space-y-2">
                ...
                {lawyers.map((lawyer, index) => (
                  <div key={lawyer.id} className="bg-card border border-border p-5 hover:border-accent transition-colors duration-200">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-[1rem] tracking-[-0.01em]" style={{ fontWeight: 600 }}>{lawyer.name}</h3>
                      {lawyer.distance && (
                        <span className="text-[0.625rem] text-accent" style={{ fontFamily: 'var(--font-mono)', fontWeight: 400 }}>{lawyer.distance}</span>
                      )}
                    </div>
                    <div className="space-y-2 mb-4">
                      <div className="flex flex-wrap gap-2">
                        {lawyer.specialties.map((specialty, idx) => (
                          <span key={idx} className="text-[0.625rem] uppercase tracking-[0.1em] px-2 py-1 bg-secondary text-secondary-foreground" style={{ fontFamily: 'var(--font-mono)', fontWeight: 400 }}>
                            {specialty}
                          </span>
                        ))}
                      </div>
